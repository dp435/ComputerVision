...
def computeSphericalWarpMappings(dstShape, f, k1, k2):
    ...
    x_norm = np.divide(x_hat, z_hat)
    y_norm = np.divide(y_hat, z_hat)

    # Apply radial distortion.
    r_sqr = x_norm**2.0 + y_norm**2.0
    xt = x_norm * (1.0 + k1*r_sqr + k2*(r_sqr**2.0))
    yt = y_norm * (1.0 + k1*r_sqr + k2*(r_sqr**2.0))
