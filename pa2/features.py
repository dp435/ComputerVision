...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        HA = ndimage.gaussian_filter(Ix*Ix, 0.5, mode='reflect')
        HB = ndimage.gaussian_filter(Ix*Iy, 0.5, mode='reflect')
        HC = ndimage.gaussian_filter(Ix*Iy, 0.5, mode='reflect')
        HD = ndimage.gaussian_filter(Iy*Iy, 0.5, mode='reflect')

        harrisImage = (HA*HD - HB*HC) - 0.1*((HA + HD)**2)
    ...
    def computeLocalMaxima(self, harrisImage):
        ...
        maximums = ndimage.maximum_filter(harrisImage,(7,7))
...
class SimpleFeatureDescriptor(FeatureDescriptor):
    ...
    def describeFeatures(self, image, keypoints):
        ...
        for i, f in enumerate(keypoints):
            ...
            imgWindow = np.zeros((5,5))
            for k in range(5):
                for j in range(5):
                    if ((x-2+k >= 0) and (x-2+k < width) and (y-2+j >= 0) and (y-2+j < height)):
                        imgWindow[j,k] = grayImage[y-2+j,x-2+k]
                    else:
                        imgWindow[j,k] = 0
            desc[i] = imgWindow.reshape(1,25)
...
class SSDFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        minIndices = np.argmin(SSD, 1) # minimum along rows

        for i in range(minIndices.size):
            matches.append(cv2.DMatch(i,minIndices[i], SSD[i , minIndices[i]]))
...
class RatioFeatureMatcher(FeatureMatcher):
    def matchFeatures(self, desc1, desc2):
        ...
        if (desc2.shape[0] > 1):
            ...
            minIndices = np.argpartition(SSD, 0) # minimum along rows
            sndMinIndices = np.argpartition(SSD, 1) # minimum along rows

            r,c = minIndices.shape
            SSD_ratio = np.zeros((r,1))
            for i in range(r):
                SSD_ratio[i,0] = np.divide(SSD[i, minIndices[i,0]], SSD[i,sndMinIndices[i,1]])

            for i in range(r):
                matches.append(cv2.DMatch(i,minIndices[i,0], SSD_ratio[i,0]))
